...
# إعداد Django لتشغيل السكربت خارج manage.py
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'ecommerce.settings')
django.setup()

from accounts.models import CustomUser
from django.core.files import File
from products.models import Product, category  # ← التطبيق = products

# ✅ ألصق بيانات المنتجات هنا داخل المصفوفة
eproducts = [    
    {
      "id": 1,
...
# تنفيذ الإضافة
for pdata in eproducts:
    cat_name = pdata['category']
    cat_obj, _ = category.objects.get_or_create(name=cat_name)

    image_url = pdata.get('thumbnail') or (pdata['images'][0] if pdata.get('images') else None)
    image_file = None
    ...
    if image_url:
        try:
            ...
            print(f"⚠️ تعذر تحميل الصورة لمنتج: {pdata['title']} - {e}")
    ...
    product = Product.objects.create(
        ...
        category=cat_obj,
        price=pdata['price'],
        discount_percentage=pdata['discountPercentage'],
        rating=pdata['rating'],
        stock=pdata['stock'],
        brand=pdata['brand']
    )
    ...
    if image_file:
        product.image.save(f"{pdata['title'].replace(' ', '_')}.webp", image_file)

    product.save()
    print(f"✅ تمت إضافة المنتج: {product.title}")
    if product.image:
        print(f"🖼️ الصورة: {product.image.url}")

print("🎉 تم استيراد جميع المنتجات.")
